...
class Client():
    ...
    def run(self):
        ...
        self.view.ddRadioButton.clicked.connect(self.insert_ddTableWidget)
        self.view.radioButton_4.clicked.connect(self.insert_ddTableWidget)
        self.view.ddTableWidget.cellClicked.connect(self.test_cell)
        # endregion
    ...
    def insert_nametablewidget(self):
        ...
        if time[0][2] != 31:
                        ...
                        for reward_1 in reward.split('|'):
                            if len(reward_1.split(',')) > 1:
                                sql = 'SELECT a.Description FROM t_s_language a,t_s_item b,t_s_item_performance c WHERE b.ItemDisplayID = c.ID ' \
                                      'AND c.NameDisplay = a.ID AND b.ID = {0}'.format(reward_1.split(',')[0])
                                # name_2 = self.select(
            ...
            # 插入活动条件为1000的TaskTarget
            sql_cond = 'select Cond from t_s_activity_task where activityid=%s' % activityID
            cond = self.select(sql_cond)
            tasktarget_list = []
            for item in cond:
                if re.match('1000', item[0]):
                    taskid = item[0].split(',')[1]
                    tasktarget = self.select('select tasktarget from t_s_task where id = %s' % taskid)[0][0]
                else:
                    tasktarget = item[0]
                tasktarget_list.append(tasktarget)
            for k in range(len(tasktarget_list)):
                self.view.rewardTableWidget.setItem(k, 4, QTableWidgetItem(tasktarget_list[k]))
        else:
            ...
            for i in range(len(malllist)):
                ...
                if mallresult:
                    ...
                    result[1] = itemlistname + result[1]
                    sql = 'SELECT a.Description FROM t_s_language a,t_s_item b,t_s_item_performance c WHERE b.ItemDisplayID = c.ID ' \
    ...
    def insert_goodslistwidget(self):
        ...
        if self.view.dropRadioButton.isChecked():
            self.view.goodsListWidget.addItem(namelike + '  ' + 'DropId')
        else:
    ...
    # 插入到宝箱表格
    def insert_goods_tablewidget(self, sql_box):
        ...
        for i in range(len(idlist)):
            ...
            if idlist[i] != 0:
                ...
                if scrtext:
                    ...
                    self.view.annTableWidget.setItem(i, 1, QTableWidgetItem('注意通知策划配置跑马灯'))
                if wortext:
                    self.view.annTableWidget.setItem(i, 2, QTableWidgetItem(wortext[0][0]))
                else:
                    self.view.annTableWidget.setItem(i, 2, QTableWidgetItem('注意通知策划配置跑马灯'))
    ...
    def insert_goodstablewidget(self, itemid):
        ...
        if len(itemlist_id) > 1:
            ...
            sql = 'SELECT a.Description FROM t_s_language a,t_s_item b,t_s_item_performance c WHERE b.ItemDisplayID = c.ID ' \
                  'AND c.NameDisplay = a.ID AND b.ID in {0}'.format(tuple(itemlist_id))
            itemnames = self.select(sql)
    ...
    # 读取更新日志
    def insert_updatetextedit(self):
        text = open('update.md')
        for line in text.readlines():
            self.view.updateTextEdit.append(line)
        # 设置初始位置在顶上
        self.view.updateTextEdit.verticalScrollBar().setValue(0)
    ...
    # region 弹弹宝藏
    def insert_ddTableWidget(self):
        self.view.ddTableWidget.setRowCount(0)
        if self.view.ddRadioButton.isChecked():
            sql = 'select ShowReward, Location, IsRare, DropId, MinLevelLimit, MaxLevelLimit, Weights, ' \
                  'RewardCoin from t_s_activity_randombox where Type = 1 order by Location'
        else:
            sql = 'select ShowReward, Location, IsRare, DropId, MinLevelLimit, MaxLevelLimit, Weights, ' \
                  'RewardCoin from t_s_activity_randombox where Type = 2 order by Location'
        result = self.select(sql)
        if not result:
            return
        self.view.ddTableWidget.setRowCount(len(result))
        # 元组转为list
        ...
        for item in result_list:
            itemid = item[0].split(',')[0]
            sql = 'select a.Description FROM t_s_language a,t_s_item b,t_s_item_performance c WHERE ' \
                  'b.ItemDisplayID = c.ID AND c.NameDisplay = a.ID AND b.ID={0}'.format(itemid)
            itemname = self.select(sql)[0][0]
            item[0] = item[0].replace(itemid, itemname)
            itemid = item[-1].split(',')[0]
            sql = 'select a.Description FROM t_s_language a,t_s_item b,t_s_item_performance c WHERE ' \
                  'b.ItemDisplayID = c.ID AND c.NameDisplay = a.ID AND b.ID={0}'.format(itemid)
            itemname = self.select(sql)[0][0]
            item[-1] = item[-1].replace(itemid, itemname)
        ...
        total = 0
        for item in result_list:
            if item[1] not in location_list:
                total += item[-2]
                location_list.append(item[1])
        for item in result_list:
            item[-2] = str(round(int(item[-2]) * 100 / total, 2)) + '%'
        for i in range(len(result_list)):
            for j in range(len(result_list[i])):
                self.view.ddTableWidget.setItem(i, j, QTableWidgetItem(str(result_list[i][j])))

    def test_cell(self):
        if self.view.ddTableWidget.currentColumn() == 3:
            self.view.tabWidget.setCurrentIndex(1)
            self.view.dropRadioButton.setChecked(True)
            self.view.goodsLineEdit.setText(self.view.ddTableWidget.currentItem().text())
            self.insert_goodslistwidget()
            self.view.goodsListWidget.setCurrentRow(0)
            self.item_click()
